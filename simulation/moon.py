...
class PlanetsSystem:
    def __init__(self, in_coll_coord, in_EPA, *args, **kwargs):
        ...
        self.earth_rotation_list = None

    def set_earth_rotation_list(self, t):
        angle_rotated = np.zeros(len(t))
        angle_rotated[0] = self.in_EPA
        for i in range(len(angle_rotated)):
            if i > 0:
                angle_rotated[i] = angle_rotated[i-1] + MOON_AV * (t[i] - t[i-1]) # rad

        self.earth_rotation_list = angle_rotated
    ...
    def coll_trajectory(self, t):
        ...
        for index, time in enumerate(t):
            if index > 0:
                ...
                rotated_position = mymath.rotate_quat(mymath.angle_axis_quat(self.earth_rotation_list[index]-self.earth_rotation_list[index-1], self.moon_rotation_axis), vector)
    ...
    def moon_surface(self, resolution = 75):
    ...
    def earth_trajectory(self, t):
        ...
        for index, time in enumerate(t):
            x[index] = -DISTANCE_MOON_EARTH* np.sin(self.earth_rotation_list[index])
            y[index] = DISTANCE_MOON_EARTH * np.cos(self.moon_angle_with_ecliptic) * np.cos(self.earth_rotation_list[index])
            z[index] = DISTANCE_MOON_EARTH * np.sin(self.moon_angle_with_ecliptic) * np.cos(self.earth_rotation_list[index])
    ...
    def sun_trajectory(self, t):
        ...
        for index, time in enumerate(t):
            x[index] = -DISTANCE_MOON_EARTH *np.cos(self.moon_angle_with_ecliptic) * np.sin(self.earth_rotation_list[index])
            y[index] = -DISTANCE_MOON_EARTH * np.cos(self.moon_angle_with_ecliptic) * np.cos(self.earth_rotation_list[index]) - DISTANCE_EARTH_SUN
